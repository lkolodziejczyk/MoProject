    ...
    for rows=1:1:miejsce
      do_mutowania(pierwszy).palletsArray(rows,1)
       ...
       for rows=(miejsce+1):1:Palety
           A(rows,1) = do_mutowania(drugi).palletsArray(rows,1);
           A(rows,2) = do_mutowania(drugi).palletsArray(rows,2);
           A(rows,3) = do_mutowania(drugi).palletsArray(rows,3);
       end;
        
    object.newMainArray = A;
end
